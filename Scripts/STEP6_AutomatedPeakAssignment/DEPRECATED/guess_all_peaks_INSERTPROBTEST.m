function [ xsel_all, d_align ] = guess_all_peaks( d, xsel );
...
insertion_probability = zeros( 1, numlanes );

[xsel_all, insertions_all, gaps_all, gap_index_all ] = align_all_peaks( ...
    d, xsel, gap_probability, insertion_probability );

[xsel, gap_probability, insertion_probability] = ...
    refine_after_first_round( xsel, xsel_all, gap_index_all,...
			      insertions_all );
...
function [xsel_new, insertions, gaps, gap_index] = ...
	...
	xsel, peaks, gap_probability, insertion_probability,...
	d_ref, d_ali );
...
GAP_PENALTY       = 5.0;
INSERTION_PENALTY = 3.5 * ( 1- insertion_probability)^3;
...
for i = 1 : num1
  for j = 1 : num2
    ...
    if ( pos1 > 0 & pos2 > 0 )
      score_aligned = score_aligned + ...
	  12.0 * (abs( ( xsel(i) - xsel(pos1)  )  - ...
		     ( peaks(j) - peaks(pos2) ) )  / ...
	  ( width * ( i - pos1 ) )).^2;
      
      % And also try a score based on matching peak intensity.
      % This looks back to last assigned peak.
      ESTIMATE = 1;
      if ESTIMATE
	score_aligned = score_aligned + ...
	...
	% Alternatively, try to guess where the immediately preceding
	% peak would be.
	current_ref_intensity = d_ref( round(xsel( i ) ) );
	previous_ref_intensity = d_ref( round(xsel( i-1 ) ) );
	current_ali_intensity = d_ali( round( peaks( j ) ) );
	previous_ali_x = interp1( [xsel(pos1) xsel(i)],...
				  [ peaks(pos2) peaks(j) ] , xsel(i-1));
	previous_ali_intensity = d_ali( round( previous_ali_x ) );
	
	score_aligned = score_aligned + ...
      	  abs( ...
	      log( current_ref_intensity/previous_ref_intensity ) -  ...
      	      log( current_ali_intensity/previous_ali_intensity ) );    
      end
...
function [xsel_all, insertions_all, gaps_all, gap_index_all ] = ...
    align_all_peaks( d, xsel, gap_probability, insertion_probability );
...
for n = 1:numlanes
  ...
  [ xsel_fit, insertions, gaps, gap_index ] = align_peaks_dynamic_programming(...
      xsel, peaks, gap_probability, insertion_probability( n ), d(:,1), d(:,n) );  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xsel, gap_probability, insertion_probability] = ...
    refine_after_first_round( xsel, xsel_all, gap_index_all, insertions_all );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for n = 1: num_lanes
  insertion_probability( n )  = length( insertions_all{n} ) / num_xsel;
end
