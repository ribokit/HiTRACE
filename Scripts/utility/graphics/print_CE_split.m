...
[xsel, seqpos] = check_monotone(xsel, seqpos);
...
for i = 1:space(2)
    for j = 1:space(1)
        ...
        xlabel('Mutation Position', 'Color', [0 1 0]); ylabel('Sequence Position', 'Color', [0 1 0]);
        if j == space(1);
        ...
        if space(6) == 1
            ...
            elseif (j == 1 && i == 2)
                ...
                title(strcat(note,' `'),'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom',...
...
if num == 1; str = 'YES'; else; str = 'NO'; end;
...
function [xsel_in, seqpos_de] = check_monotone(xsel, seqpos)

xsel_in = xsel; seqpos_de = seqpos;

fprintf('\n');
fprintf(['Input xsel (1x',num2str(length(xsel)),') is ']);
if all(diff(xsel)<0); 
    fprintf('monotonically decreasing, FLIPPED for use.\n');
    xsel_in = fliplr(xsel);
elseif all(diff(xsel)>0);
    fprintf('monotonically increasing, unchanged for use.\n');
else
    fprintf('NOT monotonic, please check.\n');
    fprintf('** Strict monotonicity required! **\n');
end;

fprintf(['Input seqpos (1x',num2str(length(seqpos)),') is ']);
if all(diff(seqpos)>0); 
    fprintf('monotonically increasing, FLIPPED for use.\n');
    seqpos_de = fliplr(seqpos);
elseif all(diff(seqpos)<0);
    fprintf('monotonically decreasing, unchanged for use.\n');
else
    fprintf('NOT monotonic, please check.\n');
    fprintf('** Strict monotonicity required! **\n');
end;
...
fprintf('\n');
fprintf(['mutpos (1x', num2str(length(mutpos)), ') is generated from d_rdat.data_annotations.\n']);