...
%      PRINT_XSEL_SPLIT(d_align, xsel, seqpos, sequence, offset, area_pred, data_types, ...
...
if ~exist('area_pred','var') || isempty(area_pred);
    ...
    area_pred = [ zeros( length(seqpos)-size(area_pred,1), size( area_pred,2)); area_pred]; % zero pad.
end;
