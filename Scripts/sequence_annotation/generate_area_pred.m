...
elseif iscell( data_types ) % input is a matrix
    if length(data_types ) > numlanes; 
        fprintf( 'WARNING! data_type has more elements than number of data traces!\n' ); 
    end;
    for m = length( data_types )+1 : numlanes 
        data_types{m} = ''; 
    end; % pad to number of lanes.
