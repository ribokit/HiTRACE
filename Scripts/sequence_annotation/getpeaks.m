function [locs,pks] = getpeaks(X,varargin)
...
lX = length(X);
for i = 1:length(pks),
    leftNeighbor = locs(i)-3:locs(i)-2;
    rightNeighbor = locs(i)+2:locs(i)+3;
    leftNeighbor(leftNeighbor < 1) = [];
    rightNeighbor(rightNeighbor > lX) = [];
    delta = min(pks(i)-mean(X(leftNeighbor)),pks(i)-mean(X(rightNeighbor)));
    if delta<Th,
