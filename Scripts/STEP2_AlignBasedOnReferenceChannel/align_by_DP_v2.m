function [d_out,x_warp_all,DP,choice] = align_by_DP_v2( d_all, refcol );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Comment out the following four lines if you dont have the
%   parallelization toolboxes.
...
for i = which_lanes;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Comment out following line if you have the parallelization
  % toolbox.
  %parfor i = which_lanes
  
  if ( i == refcol )
    %d_out(:,i) = d_all(:,i);
    x_warp_all(:,i) = [1:num_pixels];
  else
    fprintf( '-- Aligning %d of %d --\n',i,size(d_all,2));
    tic
    [d_warp, x_warp, DP, choice] = refine_by_warping( (abs(d_all(:,refcol))).^0.2, (abs(d_all(:,i))).^0.2 );
...
d_out = apply_warp( d_all, x_warp_all );



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ d_warp, x_warp, DP_matrix, choice ] = refine_by_warping( d_ref_input, d_ali );
