...
if parallelization_exists()
    parfor i = which_lanes;
        [x_transform_all(:,i), anchor_nodes(:,i)]    = align_by_DP_inner_loop( i, refcol, d_all, penalizeStretchFactor, slack, maxShift, windowSize, num_pixels, 1);
    end
else
    fprintf('\n'); revStr = ' '; fprintf(' \n');
    
    for i = which_lanes;
        [x_transform_all(:,i), anchor_nodes(:,i), revStr]    = align_by_DP_inner_loop( i, refcol, d_all, penalizeStretchFactor, slack, maxShift, windowSize, num_pixels, 0, revStr );
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  [x_transform, anchor_nodes, revStr]  = align_by_DP_inner_loop( i, refcol, d_all, penalizeStretchFactor, slack, maxShift, windowSize, num_pixels, if_parfor, revStr )

if ~exist('revStr','var'); revStr = ''; end;
...
if ( i == refcol )
    ...
    if if_parfor;
        ...
        revStr = lprintf(revStr, ['Aligning ', num2str(i), ' of ', num2str(size(d_all,2)),' ...\n'], 2);
    end;
    %tic
